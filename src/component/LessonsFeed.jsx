...
import { SearchIcon, ChevronDown, ChevronLeft, ChevronRight, Search } from "lucide-react";
...
export default function LessonsFeed({ userPlan = "Free", favorites }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 bg-default-50/30 dark:bg-gradient-to-b dark:from-[#12032e] dark:to-[#12032e] min-h-screen transition-colors duration-500">
      ...
      <form onSubmit={handleSearchSubmit} className="flex gap-3 mb-8 w-full items-center">
        <Input
          type="text"
          placeholder="Search lessons..."
          value={tempSearch}
          onChange={(e) => setTempSearch(e.target.value)}
          startContent={<SearchIcon className="text-default-400 dark:text-purple-300/40" size={18} />}
          radius="lg"
          variant="bordered"
          ...
          classNames={{
            inputWrapper: "dark:border-white/10 dark:bg-white/[0.02] dark:hover:border-purple-500/50 focus-within:!border-purple-500 h-11",
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 shrink-0 bg-white dark:bg-[#1a093c]/90 border border-slate-100 dark:border-white/[0.08] backdrop-blur-xl rounded-[24px] p-6 h-fit shadow-sm transition-all">
          <button type="button" onClick={() => setIsOpen(!isOpen)} className="md:hidden w-full flex items-center justify-between">
            <h3 className="font-bold text-sm text-slate-800 dark:text-purple-100 uppercase tracking-wider">Categories</h3>
            <ChevronDown size={18} className={`transition-transform duration-300 dark:text-purple-300 ${isOpen ? "rotate-180" : ""}`} />
          </button>
          ...
          <div className={`${isOpen ? "flex" : "hidden"} md:flex flex-col gap-2.5 mt-4 md:mt-0`}>
            {categoriesList.map((category) => {
              const isSelected = category === "All Lessons" ? selectedCategories.length === 0 : selectedCategories.includes(category);
              return (
                <button
                  type="button"
                  key={category}
                ...
                >
                  {isSelected && (
                    <motion.span layoutId="activeDot" className="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-violet-600 dark:bg-[#00e5b4]" />
                  )}
                  {category}
                </button>
              );
        ...
        <div className="flex-1">
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full h-60 flex items-center justify-center">
                <Spinner color="secondary" size="lg" label="Loading lessons..." />
              </motion.div>
            ) : lessons.length > 0 ? (
              <motion.div key="grid" variants={containerVariants} initial="hidden" animate="show" className="flex flex-col gap-y-10">
                
                {/* Lessons Card Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {lessons.map((lesson) => (
                    <motion.div key={lesson._id} variants={itemVariants} layout>
                      <LessonCard lesson={lesson} userPlan={userPlan} favorites={favorites} />
                    </motion.div>
                  ))}
                </div>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-4 pb-10">
                    <Button
                      isIconOnly variant="flat" radius="xl"
              ...
              /* Empty State */
              <motion.div key="empty" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="text-center py-20 bg-white dark:bg-[#1a093c]/50 border border-slate-100 dark:border-white/[0.06] rounded-[24px]">
                <p className="text-default-400 dark:text-purple-300/40 text-sm font-medium">No life lessons found matching your filters.</p>
                <Button
                  size="sm" variant="flat" className="mt-4 font-bold bg-violet-50 dark:bg-white/[0.04] text-violet-600 dark:text-[#00e5b4]"
                  onPress={() => {
                    setTempSearch("");
                    setSearchQuery("");
                    setSelectedCategories([]);
