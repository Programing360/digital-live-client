...
const initialReportedLessons = [
  {
    ...
    reportCount: 4,
    reports: [
      { id: "rep-1", reporter: "Nusrat Jahan", email: "nusrat@example.com", reason: "Promoting financial scams and unverified trading bot referrals." },
      { id: "rep-2", reporter: "Ried Hessan", email: "ried@example.com", reason: "Misleading high-yield investment claims without any risk disclaimers." },
      { id: "rep-3", reporter: "Sarah C.", email: "sarah@example.com", reason: "Spam content links pointing to tracking domains." },
      { id: "rep-4", reporter: "Marcus W.", email: "marcus@example.com", reason: "Dangerous advice targeting younger demographic parameters." }
    ]
  },
  {
    id: "les-402",
    ...
    reportCount: 1,
    reports: [
      { id: "rep-5", reporter: "Alex Rivera", email: "alex.r@example.com", reason: "Contains overly aggressive language frameworks counter to standard workplace ethics." }
    ]
  }
];
...
export default function ReportedLessonsPage() {
  ...
  const handleIgnoreReports = (lessonId) => {
    setReportedLessons(prev => prev.filter(l => l.id !== lessonId));
        ...
        <div className="flex items-center gap-4 bg-white dark:bg-zinc-900 border border-default-100 dark:border-zinc-800/80 p-6 rounded-[24px] shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-default-400 font-medium">
              Review flagged items submitted by the community. Cross-examine report logs and apply administrative overrides.
            </p>
          </div>
        </div>
              ...
              <Table.Content aria-label="Flagged community content table logs">
                ...
                <Table.Body>
                  {reportedLessons.map((lesson) => (
                    <Table.Row key={lesson.id} className="hover:bg-default-50/50 dark:hover:bg-zinc-800/20 transition-colors border-b border-default-100/50 dark:border-zinc-800/40 last:border-0">
                      ...
                      <Table.Cell>
                        <div className="py-1 space-y-0.5">
                          ...
                          <div className="flex items-center gap-2 text-[10px] text-default-400 font-semibold">
                            <span className="bg-default-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-default-500 capitalize">{lesson.category}</span>
                      ...
                      <Table.Cell>
                        <Chip 
                          size="sm" 
                          variant="flat" 
                          color={lesson.reportCount >= 3 ? "danger" : "warning"}
                          className={`font-black text-xs px-2.5 h-6 rounded-lg ${
                            lesson.reportCount >= 3 ? "bg-rose-500/10 text-rose-600" : "bg-amber-500/10 text-amber-600"
                          }`}
                        >
                          {lesson.reportCount} Flags
                        </Chip>
                      </Table.Cell>
                      ...
                      <Table.Cell>
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Tooltip content="Ignore Flags (Keep Live)" color="success" closeDelay={50} size="sm" className="text-xs font-bold">
                            <Button
                              isIconOnly
                              size="sm"
                              variant="flat"
                              ...
                              onPress={() => setActionConfirmation({ type: "ignore", lesson })}
                            >
                          ...
                          <Tooltip content="Permanently Delete Lesson" color="danger" closeDelay={50} size="sm" className="text-xs font-bold">
                            <Button
                              isIconOnly
                              size="sm"
                              variant="flat"
                              ...
                              onPress={() => setActionConfirmation({ type: "delete", lesson })}
                            >
            ...
            {reportedLessons.length === 0 && (
              <div className="flex flex-col items-center justify-center p-16 text-center border-t border-default-100 dark:border-zinc-800">
                ...
                <p className="text-xs text-default-400 max-w-xs mt-1 leading-relaxed">
                  Excellent! No lessons currently hold unreviewed user complaints or platform warning metrics flags.
                </p>
              </div>
            )}
            ...
            >
              ...
              <div className="p-6 overflow-y-auto space-y-4 flex-grow custom-scrollbar">
                {activeLessonDetails.reports.map((report) => (
                  <Card key={report.id} className="p-4 bg-default-50/50 dark:bg-zinc-950/40 border border-default-100 dark:border-zinc-800/40 rounded-xl space-y-2.5 flat-shadow">
                    <div className="flex items-center gap-2.5">
                      <Avatar name={report.reporter} size="sm" className="w-6 h-6 text-[10px] font-black bg-default-200 dark:bg-zinc-800" />
                      <div>
                        <p className="text-xs font-bold text-slate-700 dark:text-zinc-200">{report.reporter}</p>
                        <p className="text-[10px] text-default-400 font-medium font-sans">{report.email}</p>
                      </div>
                    </div>
                    <p className="text-xs text-default-500 dark:text-zinc-400 leading-relaxed bg-white dark:bg-zinc-900/60 border border-default-100/40 dark:border-zinc-800/30 p-2.5 rounded-lg flex items-start gap-2">
                      <MessageSquare size={12} className="text-default-400 mt-0.5 shrink-0" />
              ...
              <div className="p-4 border-t border-default-100 dark:border-zinc-800/60 bg-default-50/30 dark:bg-zinc-900/20 text-right rounded-b-[28px]">
                <Button size="sm" color="secondary" className="font-bold text-xs px-5 rounded-xl h-8" onPress={() => setActiveLessonDetails(null)}>
                  Close Logs
                </Button>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-start gap-3.5">
                <div className={`p-3 rounded-2xl ${
                  actionConfirmation.type === "delete" ? "bg-rose-500/10 text-rose-500" : "bg-emerald-500/10 text-emerald-500"
                }`}>
                  {actionConfirmation.type === "delete" ? <Trash2 size={22} /> : <CheckCircle2 size={22} />}
                </div>
                <div className="space-y-1">
                  <h3 className="text-md font-black text-slate-800 dark:text-white tracking-tight">
                    {actionConfirmation.type === "delete" ? "Confirm Account Purge" : "Ignore Log Flags"}
                  </h3>
              ...
              <div className="flex items-center justify-end gap-2 pt-1">
                <Button 
                  size="sm" variant="flat" className="font-bold text-xs h-8 px-4 rounded-xl text-default-500"
                ...
                <Button 
                  size="sm" 
                  color={actionConfirmation.type === "delete" ? "danger" : "success"}
                  className={`font-bold text-xs h-8 px-4 rounded-xl text-white ${
                    actionConfirmation.type === "delete" ? "bg-rose-600" : "bg-emerald-600"
                  }`}
                  onPress={() => actionConfirmation.type === "delete" 
                    ? handleDeleteLesson(actionConfirmation.lesson.id) 
                    : handleIgnoreReports(actionConfirmation.lesson.id)
                  }
                >
                  {actionConfirmation.type === "delete" ? "Delete Content" : "Clear Flags"}
                </Button>
              </div>
            </motion.div>
          </div>
